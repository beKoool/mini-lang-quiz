...
import { Pressable, StyleSheet, Text, View } from "react-native";
 ...
 export default function GameScreen() {
   ...
   const [isAudioLoading, setIsAudioLoading] = useState(true); // State to track audio loading
     ...
     async function prepareAudio() {
       try {
         ...
         const { sound: correctSound } = await Audio.Sound.createAsync(
           require("../assets/audio/correct.mp3") // Replace with your audio path
         );
         const { sound: wrongSound } = await Audio.Sound.createAsync(
           require("../assets/audio/wrong.mp3") // Replace with your audio path
         );

         // Set sounds
         setCorrectSound(correctSound);
         setWrongSound(wrongSound);
         setIsAudioLoading(false); // Audio loaded successfully
       } catch (error) {
         console.error("Failed to load audio", error);
         setIsAudioLoading(false); // Audio failed to load
       }
     }
   ...
   const getIncorrectTranslations = (correctTranslation: string, allVocab: { word: string; translation: string }[]) => {
     ...
     if (incorrectTranslations.length < 2) {
       //If we don't have enough incorrect options, add more.  This handles edge cases where vocab size is small
   ...
   const handleAnswer = async (answer: string) => {
     ...
     setTimeout(() => {
       ...
       if (currentQuestionIndex < questions.length - 1) {
         ...
         router.replace(`/results?score=${isCorrect ? score + 1 : score}&totalQuestions=${questions.length}`);
       }
     }, 1000);
   };
